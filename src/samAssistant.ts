...
import { startEReader, loadBookContent } from './inkAirDisplay';
import { BookService, BookResult, DownloadedBook } from './bookService';
...
export class SamAssistant {
  ...
  private currentBook: DownloadedBook | null = null;
  ...
  private async searchForBooks(session: TpaSession, query: string): Promise<void> {
    try {
  ...
  private async downloadBook(session: TpaSession, bookIndex: number): Promise<void> {
    try {
      ...
      await loadBookContent(book.content);
